...
const url: string = "http://localhost:2030";
...
export const sendAccountOpeningMail = async (user: any, tokenID: string) => {
  try {
    ...
    const transport: any = nodemailer.createTransport({
      ...
      auth: {
        type: "OAuth2",
        user: "anthonyonyemaobi76@gmail.com",
        clientId: GOOGLE_SECRET,
    ...
    const data = await ejs.renderFile(readData, passesData);

    const mailer = {
      from: "Congrate🍡🍡🍡<anthonyonyemaobi76@gmail.com>",
      to: user.email,
      subject: "Awesome",
      html: data,
    };
